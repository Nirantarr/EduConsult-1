...
const Navbar = () => {
    ...
    const AuthLinks = () => (
        <>
            {userInfo.role === 'admin' && (
                <DropdownItem icon={LayoutDashboard} text="Admin Dashboard" path="/adminDashboard" />
            )}
            {userInfo.role === 'faculty' && (
                <DropdownItem icon={User} text="Faculty Profile" path="/facultyProfile" />
            )}
            {userInfo.role === 'student' && (
                <DropdownItem icon={User} text="Student Profile" path="/studentProfile" />
            )}
    ...
    return (
        ...
        //         <div className="container mx-auto flex justify-between items-center p-2">
        //             <Link to="/" className="flex items-center magnetic -my-2 -ml-2">
        //                 <img src="/logo.jpg" alt="Tajpe Logo" className="h-16 w-16 md:h-20 md:w-20 rounded-full object-cover" />
        //                 <span className="ml-2 text-2xl md:text-3xl font-bold text-primary tracking-tight font-serif">Tajpe</span>
        //             </Link>
        ...
        //             <div className="flex items-center space-x-3">
        //                 {/* Auth Dropdown is hidden on mobile, its content will be in the mobile menu */}
        //                 <div className="relative hidden md:block" ref={authDropdownRef}>
        //                     <button onClick={() => setIsAuthDropdownOpen(!isAuthDropdownOpen)}
        //                         className="magnetic flex items-center space-x-2 text-text-primary focus:outline-none focus:ring-2 focus:ring-primary/50 focus:ring-offset-2 rounded-full p-2.5 transition-all duration-300 bg-white/50 hover:bg-white/80 shadow-sm border border-transparent hover:border-white/20">
        //                         <User className="h-5 w-5 text-primary" />
        ...
        //                 <MobileNavLink href="/">Home</MobileNavLink>
        //                 <MobileNavLink href="/browse">Listings</MobileNavLink>
        //                 <MobileNavLink href="#services">Pricing</MobileNavLink>
        //                 <MobileNavLink href="/support">Support</MobileNavLink>
        ...
        //                     className="flex items-center px-4 py-3 text-base font-semibold text-text-primary hover:bg-secondary hover:text-primary transition-colors duration-200 rounded-lg">
        //                     <LogIn className="mr-3 h-5 w-5 text-text-secondary" />
        //                     Student Login
        //                 </Link>
        //                  <Link to="/student/signup" onClick={() => setIsMobileMenuOpen(false)}
        //                     className="flex items-center px-4 py-3 text-base font-semibold text-text-primary hover:bg-secondary hover:text-primary transition-colors duration-200 rounded-lg">
        //                     <UserPlus className="mr-3 h-5 w-5 text-text-secondary" />
        //                     Student Signup
        //                 </Link>

        //                 <div className="h-px bg-gray-200/80 my-2"></div>

        //                 <Link to="/faculty/login" onClick={() => setIsMobileMenuOpen(false)}
        //                     className="flex items-center px-4 py-3 text-base font-semibold text-text-primary hover:bg-secondary hover:text-primary transition-colors duration-200 rounded-lg">
        //                      <LogIn className="mr-3 h-5 w-5 text-text-secondary" />
        //                     Mentor Login
        //                 </Link>
        //                  <Link to="/faculty/signup" onClick={() => setIsMobileMenuOpen(false)}
        //                     className="flex items-center px-4 py-3 text-base font-semibold text-text-primary hover:bg-secondary hover:text-primary transition-colors duration-200 rounded-lg">
        //                     <UserPlus className="mr-3 h-5 w-5 text-text-secondary" />
        //                     Mentor Signup
        //                 </Link>
