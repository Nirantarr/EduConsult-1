            ...
            <form onSubmit={onSubmit}>
                ...
                <div className={`space-y-6 ${activeTab !== 'profile' && 'hidden'}`}>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div><label className="block text-sm font-semibold text-gray-600 mb-2">Full Name</label><div className="relative"><UserIcon className="w-5 h-5 absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" /><input name="fullName" type="text" value={profileData.fullName || ''} onChange={handleInputChange} className="w-full pl-12 pr-4 py-3 border rounded-lg"/></div></div>
                        <div><label className="block text-sm font-semibold text-gray-600 mb-2">Current Position/Title</label><div className="relative"><BriefcaseIcon className="w-5 h-5 absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" /><input name="title" type="text" value={profileData.title || ''} onChange={handleInputChange} className="w-full pl-12 pr-4 py-3 border rounded-lg"/></div></div>
                    </div>
                    <div><label className="block text-sm font-semibold text-gray-600 mb-2">Highest Education</label><div className="relative"><AcademicCapIcon className="w-5 h-5 absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" /><input name="education" type="text" value={profileData.education || ''} onChange={handleInputChange} className="w-full pl-12 pr-4 py-3 border rounded-lg"/></div></div>
                    <div><label className="block text-sm font-semibold text-gray-600 mb-2">Profile Bio/Introduction</label><textarea name="bio" value={profileData.bio || ''} onChange={handleInputChange} rows="4" className="w-full px-4 py-3 border rounded-lg" placeholder="A compelling summary..."></textarea></div>
                    <div>
                        ...
                        <form onSubmit={handleAddTag} className="flex gap-2"><input type="text" value={newTag} onChange={(e) => setNewTag(e.target.value)} placeholder="Add a new skill..." className="w-full px-4 py-2 border rounded-lg"/><button type="submit" className="bg-slate-200 text-slate-600 font-semibold px-4 rounded-lg hover:bg-slate-300">+</button></form>
                    </div>
                </div>

                {/* --- REMOVED: "Services & Availability" Tab Content --- */}
                 ...
                 <div className={`space-y-6 ${activeTab !== 'financial' && 'hidden'}`}>
                    <div>
                        ...
                        <select id="payoutMethod" name="payoutMethod" value={profileData.payoutMethod || 'paypal'} onChange={handleInputChange} className="w-full px-4 py-3 border rounded-lg">
                            <option value="paypal">PayPal</option>
                            <option value="bank">Bank Transfer</option>
                        </select>
                    </div>
                    {profileData.payoutMethod === 'paypal' && (
                        <div><label className="block text-sm font-semibold text-gray-600 mb-2">PayPal Email</label><div className="relative"><AtSymbolIcon className="w-5 h-5 absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" /><input name="paypalEmail" type="email" value={profileData.paypalEmail || ''} onChange={handleInputChange} className="w-full pl-12 pr-4 py-3 border rounded-lg" placeholder="your.email@paypal.com"/></div></div>
                    )}
                    {profileData.payoutMethod === 'bank' && (
                        <div className="space-y-4 p-4 border rounded-lg bg-slate-50">
                             <h4 className="font-semibold text-gray-700">Bank Account Details</h4>
                             <div><label className="block text-xs font-semibold text-gray-500 mb-1">Account Holder Name</label><input name="bankAccountName" type="text" value={profileData.bankAccountName || ''} onChange={handleInputChange} className="w-full px-4 py-2 border rounded-md"/></div>
                             <div><label className="block text-xs font-semibold text-gray-500 mb-1">Account Number</label><input name="bankAccountNumber" type="text" value={profileData.bankAccountNumber || ''} onChange={handleInputChange} className="w-full px-4 py-2 border rounded-md"/></div>
                             <div><label className="block text-xs font-semibold text-gray-500 mb-1">Bank Name / Routing Number</label><input name="bankRoutingNumber" type="text" value={profileData.bankRoutingNumber || ''} onChange={handleInputChange} className="w-full px-4 py-2 border rounded-md"/></div>
                        </div>
                    )}
                </div>
